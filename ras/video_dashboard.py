...
def is_arm_platform():
    try:
        ...
        machine = os.getenv("PROCESSOR_ARCHITECTURE", "").lower()
    return "arm" in machine or "aarch64" in machine


IS_ARM = is_arm_platform()


def pick_default_video_path():
    candidates = [
        os.path.join(BASE_DIR, "video.mp4"),
        os.path.join(BASE_DIR, "Video Project 5.2.mp4"),
        os.path.join(BASE_DIR, "..", "video.mp4"),
        os.path.join(BASE_DIR, "..", "Video Project 5.2.mp4"),
    ]
    for path in candidates:
        if os.path.exists(path):
            return path
    return os.path.join(BASE_DIR, "video.mp4")
...
MODEL_PATH = os.getenv("MODEL_PATH", os.path.join(BASE_DIR, "best_model.h5"))
WORDLIST_PATH = os.getenv("WORDLIST_PATH", os.path.join(BASE_DIR, "turizm_sozluk.json"))
VIDEO_PATH = os.getenv("VIDEO_PATH", pick_default_video_path())
CONFIDENCE_THRESHOLD = float(os.getenv("CONFIDENCE_THRESHOLD", "0.60"))
COMMIT_CONFIDENCE_THRESHOLD = float(
    os.getenv("COMMIT_CONFIDENCE_THRESHOLD", "0.15" if IS_ARM else "0.20")
)
...
PROCESS_EVERY_N_FRAME = int(os.getenv("PROCESS_EVERY_N_FRAME", "4" if IS_ARM else "2"))
TOP_K = int(os.getenv("TOP_K", "5"))
SMOOTHING_WINDOW = int(os.getenv("SMOOTHING_WINDOW", "3" if IS_ARM else "5"))
DOMINANT_MIN_COUNT = int(os.getenv("DOMINANT_MIN_COUNT", "1" if IS_ARM else "2"))
MIN_COMMIT_INTERVAL_SEC = float(
    os.getenv("MIN_COMMIT_INTERVAL_SEC", "0.30" if IS_ARM else "0.45")
)
NOTHING_RELEASE_REQUIRED = int(os.getenv("NOTHING_RELEASE_REQUIRED", "2"))
STREAM_FRAME_INTERVAL_SEC = float(
    os.getenv("STREAM_FRAME_INTERVAL_SEC", "0.06" if IS_ARM else "0.03")
)
STREAM_MAX_WIDTH = int(os.getenv("STREAM_MAX_WIDTH", "960" if IS_ARM else "1280"))
JPEG_QUALITY = int(os.getenv("JPEG_QUALITY", "75" if IS_ARM else "85"))
...
def process_video_loop():
    ...
    while True:
        ...
        if STREAM_MAX_WIDTH > 0 and overlay.shape[1] > STREAM_MAX_WIDTH:
            ratio = STREAM_MAX_WIDTH / float(overlay.shape[1])
            resized_h = max(1, int(overlay.shape[0] * ratio))
            overlay = cv2.resize(overlay, (STREAM_MAX_WIDTH, resized_h))

        ok, encoded = cv2.imencode(
            ".jpg",
            overlay,
            [int(cv2.IMWRITE_JPEG_QUALITY), max(20, min(95, JPEG_QUALITY))],
        )
        if ok:
            with frame_lock:
